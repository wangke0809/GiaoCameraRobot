...
def crop_thumbnail(image, bounding_box, padding=1, size=100):
    ...
    if p1x < 0:
        img = cv2.copyMakeBorder(img, top=0, bottom=0, left=-p1x, right=0, borderType=cv2.BORDER_CONSTANT,
                                 value=[0, 0, 0])
        x1 -= p1x
        x2 -= p1x
        p2x -= p1x
        p1x -= p1x
    if p1y < 0:
        img = cv2.copyMakeBorder(img, top=-p1y, bottom=0, left=0, right=0, borderType=cv2.BORDER_CONSTANT,
                                 value=[0, 0, 0])
        y1 -= p1y
        y2 -= p1y
        p2y -= p1y
        p1y -= p1y
    if p2x > w:
        img = cv2.copyMakeBorder(img, top=0, bottom=0, left=0, right=p2x - w, borderType=cv2.BORDER_CONSTANT,
                                 value=[0, 0, 0])
    if p2y > h:
        img = cv2.copyMakeBorder(img, top=0, bottom=p2y - h, left=0, right=0, borderType=cv2.BORDER_CONSTANT,
                                 value=[0, 0, 0])
